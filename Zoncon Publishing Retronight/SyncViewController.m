...
- (void)uploadViews {
    ...
    if([self checkIfDBPopulated]) {
        ...
        for(int i = 0; i < arr.count; i++) {
            ...
            NSString *idStream = [cv objectForKey:DB_COL_TITLE];
            NSString *idItem = [cv objectForKey:DB_COL_SUBTITLE];
            NSString *views = [cv objectForKey:DB_COL_NAME];
            
            NSString *urlString = [NSString stringWithFormat:@"%@%@%@", SERVER, API, API_ADD_ITEM_VIEWS];
            NSString *myRequestString = [NSString stringWithFormat:@"params=[{\"idProject\": \"%@\", \"email\": \"%@\", \"idStream\": \"%@\", \"idItem\": \"%@\", \"count\": \"%@\"}]", PID, @"", idStream, idItem, views];
            ...
            cv = @{
                   DB_COL_TYPE: DB_RECORD_TYPE_ITEM_VIEWS,
                   DB_COL_TITLE: idStream,
                   DB_COL_SUBTITLE: idItem
                   };
...
- (Boolean)populateMyLocation {
    ...
    MY_COUNTRYID = @"1";
    MY_COUNTRYNAME = @"India";
    MY_STATEID = @"13";
    MY_STATENAME = @"Maharashtra";
    MY_CITYID = @"147";
    MY_CITYNAME = @"Pune";
    return true;
